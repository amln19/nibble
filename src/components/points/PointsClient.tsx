...
type CategoryMeta = { label: string; Icon: (props: { size?: number; className?: string }) => ReactNode };

const CATEGORY_META: Record<AchievementCategory, CategoryMeta> = {
  saver:   { label: "Recipe Saver",  Icon: BookmarkCheck },
  creator: { label: "Creator",       Icon: Camera },
  chef:    { label: "Cook-Along",    Icon: ChefHat },
          ...
          <div className="flex items-center gap-2">
            <p
              className={`text-sm font-extrabold leading-tight ${
                unlocked ? "text-primary-dark" : "text-foreground"
              }`}
            >
              {achievement.title}
            </p>
            <span
              className={`ml-auto shrink-0 rounded-lg border px-1.5 py-0.5 text-[10px] font-extrabold tabular-nums ${
                unlocked ? "border-primary text-primary-dark" : "border-edge text-muted"
              }`}
            >
...
export function PointsClient() {
  ...
  if (!loggedIn) {
    return (
      <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col items-center justify-center px-4 py-24 text-center">
        <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl border-2 border-primary bg-primary-light shadow-[0_4px_0_var(--primary)]">
          <Trophy size={24} className="text-primary-dark" />
        </div>
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 pt-6 pb-28 sm:px-6 lg:px-8 md:pb-8">
      ...
      {/* ── Rank card ── */}
      <div className="overflow-hidden rounded-2xl border-2 border-primary bg-primary-light shadow-[0_4px_0_var(--primary)]">
        <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:gap-6">
          {/* Tier icon */}
          <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl border-2 border-primary bg-primary/15">
            <Trophy size={26} className="text-primary-dark" />
          </div>

          <div className="flex-1">
            <p className="text-xs font-extrabold uppercase tracking-widest text-primary-dark/60">
              Current rank
            </p>
            <p className="mt-0.5 text-xl font-extrabold text-primary-dark">{tier.label}</p>
            <p className="text-sm text-primary-dark/70">{tier.tagline}</p>
          </div>

          <div className="text-left sm:text-right">
            <p className="text-3xl font-extrabold tabular-nums text-primary-dark">
              {totalPoints}
              <span className="ml-1 text-base font-bold text-primary-dark/60">pts</span>
            </p>
            <p className="text-xs font-bold text-primary-dark/50">{totalPossible} pts possible</p>
          </div>
        </div>

        {/* Progress to next tier */}
        {tier.nextTier && (
          <div className="border-t-2 border-primary/40 px-5 py-3">
            <div className="mb-2 flex justify-between text-[11px] font-extrabold text-primary-dark/60">
              <span>{tier.label}</span>
              <span>{tier.nextTier.label} — {tier.nextTier.minPoints} pts</span>
            </div>
            <div className="h-1.5 w-full overflow-hidden rounded-full bg-primary/25">
              <div
                className="h-full rounded-full bg-primary-dark transition-all duration-700"
      ...
      <div className="mt-4 grid grid-cols-5 gap-2">
        {TIERS.map((t) => {
          ...
          return (
            ...
            >
              <span className={`text-[10px] font-extrabold leading-tight ${isCurrent ? "text-primary-dark" : "text-foreground"}`}>
                {t.label}
              </span>
              <span className={`text-[9px] font-bold ${isCurrent ? "text-primary-dark/70" : "text-muted"}`}>
                {t.minPoints}+
              </span>
            </div>
          );
      ...
      <div className="mt-6 grid grid-cols-4 gap-3">
        {(
          [
            { label: "Saved",   value: stats!.savedCount,    Icon: BookmarkCheck },
            { label: "Posts",   value: stats!.postsCount,    Icon: Camera },
            { label: "Cooked",  value: stats!.cookSessions,  Icon: UtensilsCrossed },
