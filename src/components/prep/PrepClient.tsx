...
import {
  prepSimulationCompleteSpeech,
  prepSimulationFeedbackSpeech,
  prepSimulationIntroSpeech,
  prepSimulationStepSpeech,
} from "@/lib/gordon/prep-simulation-speech";
import { useGordonSpeak } from "@/hooks/useGordonSpeak";
...
export function PrepClient({ recipeId }: { recipeId: string | null }) {
  ...
  const [voiceMuted, setVoiceMuted] = useLocalStorageState(
    "nibble-prep-simulation-voice-muted",
    false,
  );
  const [voiceNeedsAuth, setVoiceNeedsAuth] = useState(false);
  const { speak, stop, isSpeaking } = useGordonSpeak(voiceMuted);

  const voiceToggleClass = `inline-flex items-center gap-1.5 rounded-xl border-2 px-3 py-1.5 text-xs font-extrabold shadow-[0_2px_0_var(--edge)] transition-all active:translate-y-0.5 active:shadow-none ${
    voiceMuted
      ? "border-edge bg-card text-muted"
      : "border-primary bg-primary-light text-primary-dark shadow-[0_2px_0_var(--primary)]"
  }`;

  const voiceActionClass =
    "inline-flex items-center gap-1.5 rounded-xl border-2 border-edge bg-card px-3 py-1.5 text-xs font-extrabold text-foreground shadow-[0_2px_0_var(--edge)] transition-all hover:border-edge-hover disabled:pointer-events-none disabled:opacity-40 active:translate-y-0.5 active:shadow-none";

  const speakTracked = useCallback(
    async (text: string) => {
      const r = await speak(text);
      if (!r.ok && "status" in r && r.status === 401) {
        setVoiceNeedsAuth(true);
      }
    },
    [speak],
  );
  ...
  useEffect(() => {
    if (phase !== "playing" || !simulation || voiceMuted) return;
    if (stepIndex === 0) return;
    const step = simulation.steps[stepIndex];
    if (!step) return;
    void speakTracked(
      prepSimulationStepSpeech(stepIndex, simulation.steps.length, step),
    );
  }, [phase, simulation, stepIndex, voiceMuted, speakTracked]);

  useEffect(() => {
    if (!feedback || !simulation || voiceMuted) return;
    const step = simulation.steps[stepIndex];
    if (!step) return;
    void speakTracked(prepSimulationFeedbackSpeech(feedback, step));
  }, [feedback, simulation, stepIndex, voiceMuted, speakTracked]);

  useEffect(() => {
    if (phase !== "complete" || !simulation || !recipe || voiceMuted) return;
    if (completionSpokenRef.current) return;
  ...
  useEffect(() => {
    if (phase !== "complete") completionSpokenRef.current = false;
  }, [phase]);
  ...
  function startPrep() {
    ...
    setResults([]);
    setFeedback(null);
    if (simulation && !voiceMuted) {
      const first = simulation.steps[0];
      if (first) {
        void speakTracked(
          prepSimulationStepSpeech(0, simulation.steps.length, first),
        );
      }
    }
  }

  const replayStepNarration = useCallback(() => {
    if (!simulation || voiceMuted) return;
    const step = simulation.steps[stepIndex];
    if (!step) return;
    void speakTracked(
      prepSimulationStepSpeech(stepIndex, simulation.steps.length, step),
    );
  }, [simulation, stepIndex, voiceMuted, speakTracked]);

  const replayIntroNarration = useCallback(() => {
    if (!recipe || !simulation || voiceMuted) return;
    void speakTracked(prepSimulationIntroSpeech(recipe, simulation));
  }, [recipe, simulation, voiceMuted, speakTracked]);

  const replayCompleteNarration = useCallback(() => {
    if (!recipe || !simulation || voiceMuted) return;
    void speakTracked(
      prepSimulationCompleteSpeech(
        recipe,
  ...
  }, [recipe, simulation, perfectCount, totalSteps, voiceMuted, speakTracked]);
        ...
        <div className="flex min-h-dvh flex-col items-center justify-center gap-6 p-6">
          ...
          <div className="text-center">
            <h2 className="text-xl font-extrabold text-foreground">
              Setting up the virtual kitchen…
            </h2>
            <p className="mt-2 text-sm text-muted">
              Gordon is preparing your cooking simulation
            </p>
          </div>
    ...
    return (
      <div className="prep-mode-root">
        <div className="flex min-h-dvh flex-col">
          <div className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b-2 border-edge bg-background px-4 py-3">
            <Link
              href="/"
              onClick={() => stop()}
            ...
            <div className="flex flex-wrap items-center justify-end gap-2">
              <button
                ...
                className={voiceToggleClass}
                aria-label={voiceMuted ? "Unmute practice narration" : "Mute practice narration"}
              >
                {voiceMuted ? <VolumeX size={13} /> : <Volume2 size={13} />}
                {voiceMuted ? "Muted" : "Sound"}
              </button>
              {isSpeaking ? (
                <button
                  type="button"
                  onClick={() => stop()}
                  className={voiceActionClass}
                >
                  Stop
                </button>
              ) : null}
              <button
                type="button"
                disabled={voiceMuted}
                onClick={() => void replayIntroNarration()}
                className={voiceActionClass}
              >
                Hear intro
              </button>
          ...
          {voiceNeedsAuth ? (
            <p className="border-b border-edge bg-primary-light/40 px-4 py-2 text-center text-xs font-bold text-primary-dark dark:bg-primary-light/25">
              Sign in to hear Gordon during practice.
            </p>
          ) : null}
          ...
          <div className="flex flex-1 flex-col items-center gap-5 overflow-y-auto px-6 pb-10 pt-6">
            ...
            <div className="text-center">
              ...
              <p className="mt-1 text-sm font-bold text-primary">
                Virtual Kitchen · {totalSteps} steps
              </p>
            </div>
            ...
            <div className="w-full max-w-md space-y-2">
              {simulation.steps.map((s, i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 rounded-2xl border-2 border-edge bg-card px-4 py-2.5 shadow-[0_2px_0_var(--edge)]"
                >
    ...
    return (
      <div className="prep-mode-root">
        <div className="flex min-h-dvh flex-col">
          ...
          <div className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b-2 border-edge bg-background px-4 py-3">
            ...
            <div className="flex flex-wrap items-center justify-end gap-2">
              ...
              <button
                type="button"
                disabled={voiceMuted}
                onClick={() => void replayStepNarration()}
                className={voiceActionClass}
              >
                Replay step
              </button>
          ...
          {/* Step content */}
          <div
            className="relative flex flex-1 flex-col items-center justify-center gap-4 px-6 py-6 prep-challenge-enter"
            key={stepIndex}
          >
            {/* Step header */}
            ...
            <p className="max-w-md text-center text-sm font-bold text-muted">
            ...
            {feedback && (
              ...
              >
                ...
                <p className="text-xl font-black text-foreground">
                  {feedback.perfect ? "Perfect!" : "Good try!"}
                </p>
        ...
        <div className="flex min-h-dvh flex-col items-center gap-6 overflow-y-auto px-6 py-10">
          <div className="flex flex-wrap items-center justify-center gap-2">
            ...
            <button
              type="button"
              disabled={voiceMuted}
              onClick={() => void replayCompleteNarration()}
              className={voiceActionClass}
            >
              Replay wrap-up
            </button>
          ...
          {voiceNeedsAuth ? (
            <p className="max-w-md rounded-xl border-2 border-primary/30 bg-primary-light/40 px-4 py-2 text-center text-xs font-bold text-primary-dark dark:bg-primary-light/25">
              Sign in to hear Gordon during practice.
            </p>
          ) : null}
          ...
          <div className="relative">
            <GordonAvatar
              mood={isSpeaking ? "speaking" : "celebrating"}
              size={100}
            />
          ...
          <Link
            href="/"
            onClick={() => stop()}
            className="text-sm font-extrabold text-muted underline underline-offset-2 transition-colors hover:text-foreground"
          >
            Back to Recipes
          </Link>
        </div>
